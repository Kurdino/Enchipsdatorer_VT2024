...
/* USER CODE BEGIN PV */
uint16_t button_exti_count; //antal gånger callback körts
...
void button_mode()
{
	...
	while (1)
	{
		//uint16_t curr_tick = Haltick();
		//if(curr_tick - last_tick > 20) {}
		...
		if (unhandled_exti) // was set in interrupt
		{
			...
			//uint16_t curr_tick = HAL_GetTick();
			//if(curr_tick - last_tick > BOUNCE_DELAY_MS)
